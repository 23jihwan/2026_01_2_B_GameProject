...
public class JsonTorScriptableConverter : EditorWindow
{
    ...
    {
        GetWindow<JsonTorScriptableConverter>("JSON to Scriptable Objects");
    }

    public void ConvertJsonToScriptableObjects()
    {
        //폴더 생성
        ...
        {
            //JSON 파싱
            List<ItemData> itmeDataList = JsonConvert.DeserializeObject<List<ItemData>>(jsonText);

            List<ItemSO>createdItems =new List<ItemSO>();

            foreach (ItemData itemData in itmeDataList)
            {
                ...
                itemSO.isStackedble = itemData.isStackedble;
    ...
    void OnGUI()
    {
        GUILayout.Label("JSON to Scriptabel object Converter", EditorStyles.boldLabel);
        ...
        if (GUILayout.Button("Convert to Scriptable Objects"))
        {
            ...
            {
                EditorUtility.DisplayDialog("Error", "Pease Selet a JSON file first", "OK");
                return;
            }
